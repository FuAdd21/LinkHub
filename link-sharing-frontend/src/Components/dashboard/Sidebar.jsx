...
export default function Sidebar({ isOpen, onClose, user, links }) {
  ...
  return (
    <>
      ...
      <aside
        className={cx(
          "fixed inset-y-0 left-0 z-50 flex w-[284px] flex-col border-r border-white/10 bg-[#09101d]/95 px-4 py-5 shadow-2xl backdrop-blur-xl transition-transform duration-300 lg:sticky lg:translate-x-0",
          isOpen ? "translate-x-0" : "-translate-x-full",
        )}
      >
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#6366F1,#22C55E)] font-display text-lg font-semibold text-white shadow-[0_20px_40px_rgba(99,102,241,0.28)]">
              L
            </div>
            <div>
        ...
        <div className="mt-8 rounded-[28px] border border-white/10 bg-white/5 p-4">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-400">
            Add a bio, links, and connected socials to complete the creator setup.
          </p>
        </div>
                ...
                {({ isActive }) => (
                  <div
                    className={cx(
                      ...
                      isActive
                        ? "bg-white text-white shadow-[0_18px_45px_rgba(15,23,42,0.25)]"
        ...
        <a
          ...
          className="mt-4 flex items-center justify-between rounded-[26px] border border-white/10 bg-[linear-gradient(145deg,rgba(99,102,241,0.16),rgba(34,197,94,0.08))] px-4 py-4 text-sm text-white transition hover:border-white/20"
        >
          <div>
            <p className="font-semibold">View public page</p>
            <p className="mt-1 text-xs text-slate-300">
              {user?.username ? `linkhub.com/${user.username}` : "Set a username first"}
            </p>
          </div>
