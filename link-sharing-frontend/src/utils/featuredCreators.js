...
const THEME_PRESETS = {
  "dark-pro": {
    label: "Dark Pro",
    phoneShell:
      "linear-gradient(180deg, rgba(17, 24, 39, 0.95) 0%, rgba(2, 6, 23, 0.98) 100%)",
    ...
    headerGlow:
      "radial-gradient(circle at top, rgba(99, 102, 241, 0.32), transparent 62%)",
    accent: "#8b5cf6",
    accentGlow:
      "radial-gradient(circle, rgba(99, 102, 241, 0.45), transparent 72%)",
    text: "#f8fafc",
  ...
  "glass-dark": {
    label: "Glass Dark",
    phoneShell:
      "linear-gradient(180deg, rgba(24, 34, 54, 0.96) 0%, rgba(9, 14, 28, 0.98) 100%)",
    phoneBorder: "rgba(255, 255, 255, 0.14)",
    notch: "rgba(10, 15, 25, 0.92)",
    screenBackground:
      "linear-gradient(180deg, rgba(11, 20, 35, 0.86) 0%, rgba(26, 38, 62, 0.92) 100%)",
    screenBorder: "rgba(255, 255, 255, 0.09)",
    headerGlow:
      "radial-gradient(circle at top, rgba(56, 189, 248, 0.28), transparent 58%)",
    accent: "#38bdf8",
    accentGlow:
      "radial-gradient(circle, rgba(56, 189, 248, 0.42), transparent 72%)",
    text: "#eff6ff",
  ...
  "neon-glow": {
    label: "Neon Theme",
    phoneShell:
      "linear-gradient(180deg, rgba(10, 6, 28, 0.98) 0%, rgba(5, 11, 23, 0.98) 100%)",
    ...
    headerGlow:
      "radial-gradient(circle at top, rgba(0, 255, 170, 0.28), transparent 58%)",
    accent: "#00f5b0",
    accentGlow:
      "radial-gradient(circle, rgba(0, 255, 170, 0.42), transparent 72%)",
    text: "#ecfeff",
  ...
  "creator-mode": {
    ...
    phoneShell:
      "linear-gradient(180deg, rgba(44, 10, 33, 0.98) 0%, rgba(18, 10, 34, 0.98) 100%)",
    phoneBorder: "rgba(251, 146, 60, 0.16)",
    notch: "rgba(24, 8, 27, 0.92)",
    screenBackground: "linear-gradient(180deg, #240c1f 0%, #3f1330 100%)",
    screenBorder: "rgba(255, 255, 255, 0.08)",
    headerGlow:
      "radial-gradient(circle at top, rgba(251, 146, 60, 0.24), transparent 58%)",
    accent: "#fb923c",
    accentGlow:
      "radial-gradient(circle, rgba(249, 115, 22, 0.34), transparent 72%)",
    text: "#fff7ed",
...
export const THEME_SHOWCASE_PRESETS = [
  {
    id: "dark-pro",
    title: "Dark theme",
    description: "High-contrast layouts for polished creator brands.",
  },
  {
    id: "neon-glow",
    title: "Neon theme",
    description: "Electric accents for gaming, music, and culture creators.",
  },
  {
    id: "glass-dark",
    title: "Glass theme",
    description:
      "Blurred panels and layered depth for premium personal brands.",
  },
  {
    id: "minimal",
    title: "Minimal theme",
    description: "Clean surfaces for consultants, writers, and product teams.",
  },
];
...
export function normalizeCreator(creator) {
  ...
  const followers =
    Number(creator.followerCount ?? creator.stats?.followers ?? 0) || 0;
  ...
  return {
    ...
    backgroundType:
      creator.backgroundType || creator.background_type || "gradient",
    backgroundValue:
      creator.backgroundValue || creator.background_value || null,
    socials,
    links,
    stats: {
      followers,
      linkCount:
        Number(creator.stats?.linkCount ?? links.length) || links.length,
      socialCount:
        Number(creator.stats?.socialCount ?? socials.length) || socials.length,
    },
...
export function resolveFeaturedCreators(creators, limit = 3) {
  ...
  const baseCreators = normalizedCreators.length
    ? normalizedCreators
    : fallbackCreators;
...
export function getCreatorTheme(theme, backgroundType, backgroundValue) {
  ...
  return {
    ...preset,
    screenBackground: resolveBackground(
      preset,
      backgroundType,
      backgroundValue,
    ),
